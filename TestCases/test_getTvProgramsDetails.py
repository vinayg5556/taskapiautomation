...
class Tv_programs(unittest.TestCase):
    ...
    def test_get_tv_programme_details(self):
        url = str(self.baseUrl)+str(read_config('type', 'tv'))+"/"+str(read_config('ids', 'programmeTvId'))
        response = requests.get(url, self.payload)
        programmeName = jsonpath.jsonpath(response.json(), 'original_name')
        print(programmeName[0])
        # self.log.info(f"got the response status as {response.status_code} and response is {response.text} for the request getTvProgrammeDetails")
        assert response.status_code == 200
        assert programmeName[0] == "Bhagya Lakshmi"

    @pytest.mark.topRatedTvProgrammes
    def test_get_tv_top_rated_programs(self):
        url = str(self.baseUrl)+str(read_config('type', 'tv'))+"/"+str(read_config('ids', 'rated'))
        response = requests.get(url, self.payload)
    ...
    def test_get_tv_episodes(self):
        url = str(self.baseUrl)+str(read_config('type', 'tv'))+"/"+str(read_config('ids', 'episodesTvId'))+"/"+str(read_config('type', 'season'))+"/"+str(read_config('ids', 'seasonId'))+"/"+str(read_config('type', 'episode'))+"/"+str(read_config('ids', 'episodeId'))
        response = requests.get(url, self.payload)
